...
Code parse(std::istream& is) {
	...
	if (c == ')') // sub done
	{
	    ...
	    else stack.back() = cons(top, stack.back());
	...
	if (extra_type != nil) { // one of the extension hooks managed to parse the atom
	    if (stack.empty()) stack.push_back(extra_type);
	    else stack.back() = cons(extra_type, stack.back());
	    continue;
	}
	...
	if (stack.empty()) stack.push_back(name);
	else stack.back() = cons( name, stack.back());
